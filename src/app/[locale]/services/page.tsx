      ...
      <section className="bg-gradient-to-b from-surface to-white py-16 lg:py-20">
        <Container>
          <div className="mx-auto max-w-2xl text-center">
            <h1 className="text-4xl font-semibold tracking-tight text-foreground sm:text-5xl">{d.servicesPage.pageTitle}</h1>
            <p className="mt-4 text-lg text-muted leading-relaxed">{d.servicesPage.pageSubtitle}</p>
          </div>
        </Container>
      </section>
                      ...
                      <ul className="space-y-2 mb-5">
                        {service.included.slice(0, 3).map((item) => (
                          <li key={item} className="flex items-start gap-2 text-sm text-muted">
                            <Check className="h-3.5 w-3.5 mt-0.5 shrink-0 text-accent" />{item}
                          </li>
                        ))}
                      </ul>
